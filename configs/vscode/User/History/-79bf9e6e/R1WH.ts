...
describe('Verify Active Route details', () => {
  beforeEach(() => {
    ...
    //   cy.getRoutesQuery('getRouteNoForActiveRouteServiceEvent').then(query => {
    //     query = query.replace('${division}', nonIftaDivisions);
    //     cy.routesQuery(query).then(result => {
  ...
  it('should verify Active Route details', { tags: ['@smoke'] }, () => {
    ...
    // ------------------- test new functions ------------------------------------------------------
    cy.visit({url: '/', qs: {networkId: 'automation', token: Cypress.env('secrets').riseToken});
    cy.nav('activeRoutes');
    cy.setDivision('620');
    ...
    //   // Then I see success message "Additional services have been added to the driver's route and the customer's account." is displayed
