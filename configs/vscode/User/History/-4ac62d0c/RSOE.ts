...
describe('Verify Routes with data ', () => {
  beforeEach(() => {
    ...
    cy.fixture('divisions').as('divisions').then((divisions) => {
      ...
      cy.getRoutesQuery('getRouteNoForCancelRouteWithData').then((query) => {
        query = query.replace('$division', divisionNumbers);
  ...
  it(
    'are cancelled',
    { tags: ['@TC_CancelRouteWithData', '@regressionTest', '@regression'] }, function () {
