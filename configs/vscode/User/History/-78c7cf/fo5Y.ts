...
describe('Close Route Error - Midnight routes validations', () => {
  ...
  it.only(
    ...
    function () {
      ...
      const inTime = '#loads-lc-in-time-0';
      const outTime = '#loads-lc-out-time-0';
      const finishTime = '#loads-lc-finish-time-0';
      const receipt = '#loads-lc-receipt-0';
      ...
      cy.get(loadTimeTable)
        .eq(0)
        .within(() => {
          cy.get(loadTimeTable).clear();
          cy.get(loadTimeTable).type(testCaseData.startLoadOne);
          ...
          cy.get('p-calendar[formcontrolname="finishTime"]').find('input').clear();
          cy.get('p-calendar[formcontrolname="finishTime"]')
            .find('input')
            .type(testCaseData.finishLoadOne);

          if (testCaseData.receiptOne !== 'None') {
            cy.get('input[formcontrolname="receipt"]').should('be.visible');

            cy.get('input[formcontrolname="receipt"]').clear();
            cy.get('input[formcontrolname="receipt"]').type(testCaseData.receiptOne);
          } else {
            cy.get('input[formcontrolname="receipt"]').click();
          }
        });
