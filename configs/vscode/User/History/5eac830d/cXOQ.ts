...
Cypress.Commands.add('routesQuery', (queryName: string, replacements?) => {
  return cy.readFile(`cypress/fixtures/queries/routes/${queryName}.sql`).then(query => {
    if (replacements) {
      return cy.replaceQueryValues(query, replacements).then(replacedQuery => {
        return cy.task('createMySQLConnection', replacedQuery).then((recordset: any[]) => {
...
Cypress.Commands.add('replaceQueryValues', (query: string, replacements) => {
