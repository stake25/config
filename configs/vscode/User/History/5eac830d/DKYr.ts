...
Cypress.Commands.add('nav', (page: string, options?: any) => {
  // cy.visit(`/operations?networkId=automation&token=${Cypress.env('secrets').riseToken}`);
  return window.sessionStorage.getItem('lastDivision').then(division => {
    return cy.fixture('urls').then(urls => {
      let url = '';

      if (options && options.auth) {
        url = `${urls[page]}?networkId=automation&token=${Cypress.env('secrets').riseToken}`;
      } else {
        url = urls[page];
      }

      cy.visit(url);
      cy.get(`img[alt='Republic Services company logo']`, { timeout: 10000 }).should('be.visible');

      return cy.setDivision(division);
    });
  });
});
...
Cypress.Commands.add('routesQuery', (query: string) => {
  return cy.task('createMySQLConnection', query);
});
