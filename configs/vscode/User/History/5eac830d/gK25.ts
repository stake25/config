...
Cypress.Commands.add('generateDBQueryDate', (dd: string) => {
  const today = new Date();
  const mm = today.getMonth() + 1;
  const yyyy = today.getFullYear();

  const day = parseInt(dd.padStart(2, '0'));
  const month = parseInt(mm.toString().padStart(2, '0'));

  return cy.wrap(`${yyyy}-${month}-${day}`);
});

Cypress.Commands.add('nav', (page: string, division?: string) => {
  return cy.fixture('urls').then(urls => {
    if (division) {
      cy.setDivision(division);
    }
